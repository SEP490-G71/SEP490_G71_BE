...
public interface DailyQueueRepository extends JpaRepository<DailyQueue, String> {
    ...
    @Query(value = "SELECT * FROM daily_queues WHERE status = :status AND deleted_at IS NULL AND queue_date >= curdate() ORDER BY queue_date DESC LIMIT 1", nativeQuery = true)
