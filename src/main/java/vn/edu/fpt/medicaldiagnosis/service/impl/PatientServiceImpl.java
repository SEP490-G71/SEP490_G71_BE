...
public class PatientServiceImpl implements PatientService {
    ...
    public PatientResponse createPatient(PatientRequest request) {
        ...
        EmailTask emailTask = buildAccountEmailTaskForPatient(patient, accountRequest.getUsername(), accountRequest.getPassword(), TenantContext.getTenantId());
    ...
    private EmailTask buildAccountEmailTaskForPatient(Patient patient, String username, String password, String tenantId) {
        String url = "https://" + tenantId + ".datnd.id.vn/home";
        String content;
        ...
        try {
            ...
            content = String.format("Xin chào %s,\n\nTài khoản đã được tạo:\nUsername: %s\nPassword: %s\n\nĐăng nhập tại: %s\n\nTrân trọng.",
                    patient.getFullName(), username, password, url);
            log.warn("[{}] Không thể load template account-email.html: {}", tenantId, e.getMessage());
        }

        return EmailTask.builder()
                .id(UUID.randomUUID().toString())
                .emailTo(patient.getEmail())
                .subject("🧑‍⚕️ Tài khoản của bạn đã được tạo")
                .content(content)
                .retryCount(0)
                .status(Status.PENDING)
                .build();
